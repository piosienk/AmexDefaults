...
# Load data
df = pd.read_parquet(data_path + "/train_data_split.parquet")
...
int_imputer = SimpleImputer(missing_values=-9999, strategy="median", add_indicator=True).fit(df[add_int_var])
...
# save pickle
with open('./Imputers/int_imputer1.pickle', 'wb') as file:
    pickle.dump(int_imputer, file)
...
onehot_imputer = OneHotEncoder(drop="first").fit(df[add_cat_var])
# save pickle
with open('./Imputers/onehot_imputer.pickle', 'wb') as file:
    pickle.dump(onehot_imputer, file)
...
int_imputer = SimpleImputer(missing_values=-9999, strategy="median", add_indicator=False).fit(df[var_nan_impute])
# save pickle
with open('./Imputers/int_imputer2.pickle', 'wb') as file:
    pickle.dump(int_imputer, file)

df.loc[:, var_nan_impute] = int_imputer.transform(df[var_nan_impute])
df.to_parquet("./Additional_data/intermediate_train_data_split.parquet")
...
# df = pd.read_parquet("./Additional_data/intermediate_train_data_split.parquet")
...
for column in float_nan_grouping:
    new_col = column + "_miss_flag"
    float_imputer_fit = float_imputer.fit(df[column].to_numpy().reshape(-1, 1))
    float_imputed = float_imputer_fit.transform(df[column].to_numpy().reshape(-1, 1))
    # save pickle
    with open('./Imputers/float_imputed_{}.pickle'.format(column), 'wb') as file:
        pickle.dump(float_imputer_fit, file)
...
df.to_parquet(data_path + "/train_data_for_LSTM.parquet")
