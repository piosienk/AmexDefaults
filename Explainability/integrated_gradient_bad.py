...
baseline = torch.from_numpy(data_test["x"][bad_index[0], :, :]).reshape(-1, 13, 237).mean(axis=0).reshape(-1, 13, 237)

attribution_list = []
for i in range(20):
    print("run {}".format(i))
    input_test = torch.from_numpy(data_test["x"][bad_index[0][i*1000:(i+1)*1000], :, :]).reshape(-1, 13, 237)


    # baseline_bad = input_bad.mean(axis=0).reshape(-1, 13, 237)
    # baseline_good = torch.zeros(input_good.shape)

    y_bad = np.repeat(1, 1000).tolist()
    # y_good = np.repeat(0, 1500).tolist()

    attributions, delta = calculate_integrated_gradient(ig, input_test, baseline, y_bad, n_variables, 1000)
